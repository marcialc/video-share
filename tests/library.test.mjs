...
before(async () => {
  ...
  for (const migration of ['0001_library.sql', '0002_folder_shares.sql']) {
...
test('folder links expose current ready videos without a password and stop on moves or revocation', { timeout: 30000 }, async () => {
  const folder = await jsonResponse('/api/folders', { method: 'POST', data: { name: 'Shareable collection', color: 'green' } }, 201)
  const other = await jsonResponse('/api/folders', { method: 'POST', data: { name: 'Other collection' } }, 201)
  assert.equal((await request(`/api/folders/${folder.id}/share`, { method: 'POST', authenticated: false })).status, 401)
  assert.equal((await request(`/api/folders/${folder.id}/share`, { method: 'GET' })).status, 405)
  const { token } = await jsonResponse(`/api/folders/${folder.id}/share`, { method: 'POST' })
  assert.match(token, /^[a-f0-9-]{36}$/)
  assert.equal((await jsonResponse(`/api/folders/${folder.id}/share`, { method: 'POST' })).token, token)
  assert.equal((await jsonResponse('/api/library')).folders.find(item => item.id === folder.id).share_token, token)
  const publicPath = `/api/shared-folders/${token}`
  let publicFolder = await jsonResponse(publicPath, { authenticated: false })
  assert.equal(publicFolder.name, 'Shareable collection')
  assert.equal(publicFolder.color, 'green')
  assert.deepEqual(publicFolder.videos, [])
  assert.equal('id' in publicFolder, false)
  assert.equal('share_token' in publicFolder, false)

  const upload = await jsonResponse('/api/uploads', { method: 'POST', data: { name: 'Fresh clip', originalName: 'Fresh clip.mp4', size: bytes.length, mimeType: 'video/mp4', folderId: folder.id } }, 201)
  assert.deepEqual((await jsonResponse(publicPath, { authenticated: false })).videos, [])
  assert.equal((await request(`${publicPath}/videos/${upload.id}/media`, { authenticated: false })).status, 404)
  const parts = []
  for (let offset = 0; offset < bytes.length; offset += upload.chunkSize) {
    const part = bytes.subarray(offset, offset + upload.chunkSize)
    parts.push(await jsonResponse(`/api/uploads/${upload.id}/parts/${parts.length + 1}`, { method: 'PUT', body: part, headers: { 'Content-Length': String(part.length), 'Content-Type': 'application/octet-stream' } }))
  ...
  await jsonResponse(`/api/uploads/${upload.id}/complete`, { method: 'POST', data: { parts } })
  publicFolder = await jsonResponse(publicPath, { authenticated: false })
  assert.deepEqual(publicFolder.videos.map(video => video.id), [upload.id])
  assert.equal(publicFolder.videos[0].name, 'Fresh clip')
  assert.equal('object_key' in publicFolder.videos[0], false)
  assert.equal('share_token' in publicFolder.videos[0], false)
  const mediaPath = `${publicPath}/videos/${upload.id}/media`
  const range = await request(mediaPath, { authenticated: false, headers: { Range: 'bytes=4-15' } })
  assert.equal(range.status, 206)
  assert.deepEqual(Buffer.from(await range.arrayBuffer()), bytes.subarray(4, 16))
  assert.equal((await request(mediaPath, { method: 'HEAD', authenticated: false })).headers.get('content-length'), String(bytes.length))
  assert.match((await request(`${mediaPath}?download`, { authenticated: false })).headers.get('content-disposition'), /attachment/)
  assert.equal((await request(`${publicPath}/videos/${crypto.randomUUID()}/media`, { authenticated: false })).status, 404)
  await jsonResponse(`/api/videos/${upload.id}`, { method: 'PATCH', data: { folderId: other.id } })
  assert.deepEqual((await jsonResponse(publicPath, { authenticated: false })).videos, [])
  assert.equal((await request(mediaPath, { authenticated: false })).status, 404)
  await jsonResponse(`/api/videos/${upload.id}`, { method: 'PATCH', data: { folderId: folder.id, name: 'Renamed clip' } })
  assert.equal((await jsonResponse(publicPath, { authenticated: false })).videos[0].name, 'Renamed clip')
  await jsonResponse(`/api/folders/${folder.id}`, { method: 'PATCH', data: { name: 'Renamed collection' } })
  assert.equal((await jsonResponse(publicPath, { authenticated: false })).name, 'Renamed collection')
  await jsonResponse(`/api/folders/${folder.id}/share`, { method: 'DELETE' })
  assert.equal((await request(publicPath, { authenticated: false })).status, 404)
  assert.equal((await request(mediaPath, { authenticated: false })).status, 404)
  const newShare = await jsonResponse(`/api/folders/${folder.id}/share`, { method: 'POST' })
  assert.notEqual(newShare.token, token)
  await jsonResponse(`/api/folders/${folder.id}`, { method: 'DELETE' })
  assert.equal((await request(`/api/shared-folders/${newShare.token}`, { authenticated: false })).status, 404)
  assert.equal((await jsonResponse('/api/library')).videos.find(video => video.id === upload.id).folder_id, null)
  await jsonResponse(`/api/videos/${upload.id}`, { method: 'DELETE' })
  await jsonResponse(`/api/folders/${other.id}`, { method: 'DELETE' })
})
